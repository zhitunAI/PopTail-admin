...
import type { FrontendSettingsInfo } from '#/types/pop-tail';
...
usePageRefreshLoading(loadSettings);
    ...
    <div class="relative flex h-full min-h-0 flex-1 flex-col">
      ...
      <TabPane key="base" tab="基础设置">
        <Form :model="form" layout="vertical" class="max-w-220" :disabled="loading">
          <Form.Item label="Logo 上传">
            <div class="flex items-center gap-4">
              <img
                ...
                alt="logo"
              />
              <Upload :before-upload="uploadLogo" :show-upload-list="false" accept="image/*" :disabled="!canUse(STANDARD_BUTTON_LABELS.edit)">
          ...
          <Button v-if="canUse(STANDARD_BUTTON_LABELS.edit)" type="primary" :loading="saving" @click="saveSettings">保存基础设置</Button>
        </Form>
      </TabPane>
      <TabPane key="mail" tab="邮件设置">
        <Form :model="form" layout="vertical" class="max-w-220" :disabled="loading">
          ...
          <Button v-if="canUse(STANDARD_BUTTON_LABELS.edit)" type="primary" :loading="saving" @click="saveSettings">保存邮件设置</Button>
        </Form>
      </TabPane>
      </Tabs>
    </div>
