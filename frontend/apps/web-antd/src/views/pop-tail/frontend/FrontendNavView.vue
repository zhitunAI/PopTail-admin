...
import { deleteFrontendNavApi, getFrontendNavListApi, saveFrontendNavApi } from '#/api/pop-tail/admin';
...
interface FrontendNavRow extends FrontendNavItem {
  id: number;
}
...
const sourceRows = ref<FrontendNavRow[]>([]);
const drawerOpen = ref(false);
const editing = ref<FrontendNavRow | null>(null);
const form = ref<FrontendNavRow>({ id: 0, icon: 'lucide:menu', order: 1, path: '/', title: '', visible: true });
...
const [Grid, gridApi] = useVbenVxeGrid<FrontendNavRow>({
  formOptions: {
    collapsed: false,
    schema: filterSchema,
    ...
    proxyConfig: {
      ajax: {
        query: async ({ page }, formValues) => {
          ...
          try {
            const result = await getFrontendNavListApi();
            ...
            const icon = String(formValues.icon ?? '').trim().toLowerCase();
  ...
  } as VxeTableGridOptions<FrontendNavRow>,
});

function openCreate() {
  editing.value = null;
  form.value = { id: 0, icon: 'lucide:menu', order: sourceRows.value.length + 1, path: '/', title: '', visible: true };
  drawerOpen.value = true;
}

function openEdit(row: FrontendNavRow) {
  editing.value = row;
  form.value = { ...row };
  drawerOpen.value = true;
}
...
function remove(row: FrontendNavRow) {
  Modal.confirm({
    content: `确认删除导航「${row.title}」吗？`,
    onOk: async () => {
      await deleteFrontendNavApi(row.id);
      await onRefresh();
...
<template>
  <Page auto-content-height title="前端导航管理">
    <div class="relative flex h-full min-h-0 flex-1 flex-col">
      <Grid class="h-full min-h-0 flex-1" table-title="导航列表">
        ...
        <template #operation="{ row }">
          ...
          <Space v-else>
            <Button v-if="canUse(STANDARD_BUTTON_LABELS.edit)" size="small" type="link" @click="openEdit(row as FrontendNavRow)">编辑</Button>
            <Button v-if="canUse(STANDARD_BUTTON_LABELS.delete)" danger size="small" type="link" @click="remove(row as FrontendNavRow)">删除</Button>
          </Space>
        </template>
      </Grid>
    </div>
