...
function canUseAiWorkflowAction(action: AiWorkflowButtonAction) {
  const envelopeAllowed =
    !hasAiWorkflowButtonAccessEnvelope.value || hasFlexibleAiWorkflowButtonAccess(action);
  return envelopeAllowed && canUseMenuButton(AI_WORKFLOW_BUTTON_ACCESS_LABELS[action]);
}
...
<template>
  <Page
    title="AI 审核工作台"
  >
    <div class="workbench relative flex h-full min-h-0 flex-1 flex-col">
