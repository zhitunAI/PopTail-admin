...
import type { ApiInfo, MenuInfo, PolicyPath } from '#/types/pop-tail';
...
import { Checkbox, Empty, Spin, TabPane, Tabs } from 'ant-design-vue';
...
import { getPolicyPathByAuthorityId } from '#/api/pop-tail/auth';
...
const buttonPermissionMenus = ref<MenuInfo[]>([]);
const selectedButtonPermissionMap = ref<Record<number, number[]>>({});
...
const [Drawer, drawerApi] = useVbenDrawer({
  async onConfirm() {
    ...
    try {
      ...
      await saveButtonPermissions(saved.authorityId, selectedButtonPermissionMap.value);
      emit('success');
      drawerApi.close();
    } finally {
      drawerApi.unlock();
  ...
  async onOpenChange(isOpen) {
    ...
    selectedButtonPermissionMap.value = await loadButtonPermissions(source?.authorityId);
    await nextTick();
    formApi.setValues({
...
async function loadButtonPermissions(authorityId?: number) {
  const menusWithButtons = flattenMenus(permissions.value).filter(
    (item) => (item.menuBtn?.length ?? 0) > 0,
  );
  buttonPermissionMenus.value = menusWithButtons;

  if (!authorityId) {
    return Object.fromEntries(menusWithButtons.map((item) => [item.ID, []]));
  }

  const rows = await Promise.all(
    menusWithButtons.map(async (item) => {
      const result = await getAuthorityBtnApi({
        authorityId,
        menuID: item.ID,
      });
      return [item.ID, result.selected ?? []] as const;
    }),
  );
...
async function saveButtonPermissions(
  ...
  await Promise.all(
    Object.entries(selectedMap).map(([menuID, selected]) =>
      setAuthorityBtnApi({
        authorityId,
...
function menuButtonOptions(menu: MenuInfo) {
  return (menu.menuBtn ?? []).map((item) => ({
    ...
    <div class="mx-4 mt-4">
      <Tabs>
        ...
        <TabPane key="button" tab="按钮权限">
          <Spin :spinning="loadingPermissions" wrapper-class-name="w-full">
            <div v-if="buttonPermissionMenus.length === 0" class="py-8">
              <Empty description="当前菜单未配置可控按钮，请先在菜单管理中添加按钮。" />
            </div>
            <div v-else class="space-y-4">
              <div
                v-for="menu in buttonPermissionMenus"
                :key="menu.ID"
                class="rounded-lg border border-gray-200 p-4"
              >
                <div class="mb-3 flex items-center gap-2 text-sm font-medium">
                  <IconifyIcon v-if="menu.meta?.icon" :icon="menu.meta.icon" />
                  <span>{{ menu.meta?.title || menu.name }}</span>
                </div>
                <Checkbox.Group
                  v-model:value="selectedButtonPermissionMap[menu.ID]"
                  :options="menuButtonOptions(menu)"
                />
              </div>
            </div>
          </Spin>
