...
import type { SystemConfigInfo } from '#/types/gin-ai-admin';

import { computed, onMounted, reactive, ref } from 'vue';

import { Page } from '@vben/common-ui';

import { Button, Form, Input, InputNumber, Select, Switch, TabPane, Tabs, message } from 'ant-design-vue';

import { getSystemConfigApi, updateSystemConfigApi } from '#/api/gin-ai-admin/admin';
...
const STORAGE_KEY = 'gaa-system-config-extended-v1';
  ...
  <Page title="系统配置">
    <template #extra>
      <div class="flex flex-wrap items-center gap-2">
        <Button :loading="saving" :disabled="!dirty" type="primary" @click="save">保存配置</Button>
    ...
    <Tabs v-model:activeKey="activeTab">
      <TabPane key="system" tab="系统配置">
        <Form layout="vertical">
          <div class="grid grid-cols-2 gap-4">
            ...
            <Form.Item label="数据库类型"><Select v-model:value="form.system.dbType" :options="dbTypeOptions" /></Form.Item>
            ...
            <Form.Item label="限流次数"><InputNumber v-model:value="form.system.iplimitCount" class="w-full" /></Form.Item>
            <Form.Item label="限流时间"><InputNumber v-model:value="form.system.iplimitTime" class="w-full" /></Form.Item>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <Form.Item label="多点登录"><Switch v-model:checked="form.multipointEnabled" /></Form.Item>
            <Form.Item label="兼容刷新头"><Switch v-model:checked="form.compatibilityRefreshHeaders" /></Form.Item>
            <Form.Item label="使用 Redis"><Switch v-model:checked="form.system.useRedis" /></Form.Item>
            <Form.Item label="使用 Mongo"><Switch v-model:checked="form.system.useMongo" /></Form.Item>
            <Form.Item label="严格角色模式"><Switch v-model:checked="form.system.useStrictAuth" /></Form.Item>
          </div>
        </Form>
      </TabPane>
      ...
      <TabPane key="jwt" tab="JWT">
        <Form layout="vertical">
          <div class="grid grid-cols-2 gap-4">
            <Form.Item label="签名 Key"><Input.Password v-model:value="form.jwt.signingKey" autocomplete="new-password" /></Form.Item>
      ...
      <TabPane key="redis" tab="Redis">
        <Form layout="vertical">
          <div class="grid grid-cols-2 gap-4">
            ...
            <Form.Item label="密码"><Input.Password v-model:value="form.redis.password" autocomplete="new-password" /></Form.Item>
            <Form.Item label="数据库"><InputNumber v-model:value="form.redis.db" class="w-full" :min="0" /></Form.Item>
          </div>
        </Form>
      </TabPane>
      ...
      <TabPane key="email" tab="邮箱配置">
        <Form layout="vertical">
          <div class="grid grid-cols-2 gap-4">
            ...
            <Form.Item label="端口"><InputNumber v-model:value="form.email.port" class="w-full" /></Form.Item>
            <Form.Item label="昵称"><Input v-model:value="form.email.nickname" /></Form.Item>
            <Form.Item label="Secret"><Input.Password v-model:value="form.email.secret" autocomplete="new-password" /></Form.Item>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <Form.Item label="SSL"><Switch v-model:checked="form.email.isSSL" /></Form.Item>
            <Form.Item label="LoginAuth"><Switch v-model:checked="form.email.isLoginAuth" /></Form.Item>
          </div>
        </Form>
      </TabPane>
      ...
      <TabPane key="zap" tab="日志配置">
        <Form layout="vertical">
          <div class="grid grid-cols-2 gap-4">
            ...
            <Form.Item label="保留天数"><InputNumber v-model:value="form.zap.retentionDay" class="w-full" /></Form.Item>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <Form.Item label="显示行号"><Switch v-model:checked="form.zap.showLine" /></Form.Item>
            <Form.Item label="输出控制台"><Switch v-model:checked="form.zap.logInConsole" /></Form.Item>
          </div>
        </Form>
      </TabPane>
    </Tabs>
