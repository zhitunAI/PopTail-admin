...
const userDropdownTagText = computed(() => {
  const primaryRole = userStore.userInfo?.roles?.[1] || userStore.userInfo?.roles?.[0];
  return primaryRole ? String(primaryRole).replace(/^authority:/, '') : '';
});
